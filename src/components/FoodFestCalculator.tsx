...
// English and Hindi text translations for all UI strings
const UI_TRANSLATIONS = {
  en: {
    ...
    liveBadge: "LIVE FESTIVAL STALLS ACTIVE",
    ...
    recommendation0: "Tap items to plan your Shoolini Fair food trail!",
    recommendationLight: "Perfect for a light evening walk down Mall Road!",
    recommendationHeavy: "A complete local culinary tour! Wear comfortable walking shoes for Lower/Upper Bazar.",
  ...
  hi: {
    ...
    "Stir-fried noodles with crisp local mountain vegetables and spicy Himalayan sauces.": "ताज़ी पहाड़ी सब्जियों और तीखे हिमाचली सॉस के साथ तले हुए नूडल्स (चाуमीन)।",
    "Steamed vegetable or paneer dumplings served with spicy red chili chutney.": "सब्जी या पनीर के मोमोज, तीखी लाल मिर्च की चटनी के साथ परोसे जाते हैं।",
...
export default function FoodFestCalculator() {
  const { language } = useLanguage();
  const [menuItems, setMenuItems] = useState<FoodItem[]>(FALLBACK_MENU);
  const [selectedIds, setSelectedIds] = useState<string[]>([]);
  const [isLoading, setIsLoading] = useState<boolean>(true);

  // Helper to translate dynamically from the UI Translations dictionary
  ...
  // Fetch items from the dynamic API endpoint on mount
  useEffect(() => {
    async function loadMenu() {
      try {
        ...
        console.error("Failed to load food menu from database, using fallback:", err);
      } finally {
        setIsLoading(false);
      }
    }
  ...
  // Unique shops visited for route mapping (maintains selection sequence)
  const uniqueShops = Array.from(new Set(selectedItems.map((item) => translateMeta(item.shop))));

  // Smart budget recommendation feedback text
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        {!isLoading && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            {/* Left Column: Delicacies Responsive Grid */}
            <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6">
              {menuItems.map((item) => {
                ...
                return (
                  ...
                  >
                    {/* Checkbox State Toggle UI Element */}
            ...
            {/* Right Column: Sticky Budget Widget */}
            <div className="lg:col-span-4 lg:sticky lg:top-24 space-y-6">
              <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-md relative overflow-hidden">
                ...
                <h2 className="text-lg font-serif font-black text-red-950 mb-4 pb-3 border-b border-stone-100">
                  {ui.totalBudget}
                </h2>

                {/* Selected Items Itinerary List */}
                {selectedItems.length > 0 ? (
                  <div className="space-y-2 max-h-48 overflow-y-auto pr-1 mb-6">
                    {selectedItems.map((item) => (
                      <div
                        key={item.id}
                        className="flex items-center justify-between bg-stone-50/80 p-2.5 rounded-xl border border-stone-100 text-xs hover:bg-stone-50"
                      >
                ...
                {/* Dynamic Route Itinerary Generator */}
                {uniqueShops.length > 0 && (
                  <div className="mb-6 p-4 bg-red-50/30 rounded-2xl border border-red-950/5 space-y-2">
                ...
                <div className="bg-amber-50/50 rounded-2xl p-4 border border-amber-100/60 mb-6 space-y-3">
                  <div className="flex items-center justify-between text-xs text-stone-500">
                    ...
                    <span className="font-bold text-stone-800 bg-white px-2.5 py-0.5 rounded-md border border-slate-100 shadow-sm">
                ...
                <div className="bg-stone-50 rounded-2xl p-4 border border-stone-200/40 flex gap-3 items-start">
                ...
                {selectedIds.length > 0 && (
                  <button
                    ...
                    className="w-full mt-4 py-2.5 rounded-xl border border-red-200 text-red-700 text-xs font-bold hover:bg-red-50 active:scale-98 transition-all duration-200 cursor-pointer text-center block"
                  >
                    {ui.clearAll}
                  </button>
                )}
